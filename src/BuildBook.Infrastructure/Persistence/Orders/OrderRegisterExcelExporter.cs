...
public sealed class OrderRegisterExcelExporter(
    IOrderRegisterReader orderRegisterReader) : IOrderRegisterExcelExporter
{
    ...
    private static string BuildWorksheetXml(IReadOnlyList<OrderRegisterRow> rows)
    {
        var builder = new StringBuilder();
        builder.Append("<?xml version=\"1.0\" encoding=\"UTF-8\"?>");
        builder.Append("<worksheet xmlns=\"http://schemas.openxmlformats.org/spreadsheetml/2006/main\"><sheetData>");
        AppendRow(builder, 1, [.. OrderRegisterExportProjection.Headers]);

        for (var index = 0; index < rows.Count; index++)
        {
            AppendRow(builder, index + 2, OrderRegisterExportProjection.Project(rows[index]));
        }
