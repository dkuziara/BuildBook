...
public sealed class OrderReportExcelExporter(
    IOrderReportReader orderReportReader) : IOrderReportExcelExporter
{
    ...
    private static string BuildWorksheetXml(IReadOnlyList<OrderReportRow> rows)
    {
        var builder = new StringBuilder();
        builder.Append("<?xml version=\"1.0\" encoding=\"UTF-8\"?>");
        builder.Append("<worksheet xmlns=\"http://schemas.openxmlformats.org/spreadsheetml/2006/main\"><sheetData>");
        AppendRow(builder, 1, [.. OrderReportExportProjection.Headers]);

        for (var index = 0; index < rows.Count; index++)
        {
            AppendRow(builder, index + 2, OrderReportExportProjection.Project(rows[index]));
        }
    ...
    private static void AppendRow(StringBuilder builder, int rowNumber, params string?[] values)
    {
        ...
        {
            AppendCell(builder, rowNumber, index, values[index]);
        }
    ...
    {
        builder.Append($"<c r=\"{GetCellReference(columnIndex, rowNumber)}\" t=\"inlineStr\"><is><t");
    ...
    private const string WorkbookXml =
        ...
        <workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"
          ...
          <sheets>
            <sheet name="Order Reports" sheetId="1" r:id="rId1"/>
          </sheets>
        </workbook>
        """;
