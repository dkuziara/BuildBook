...
public sealed class RmaReportExcelExporter(
    IRmaReportReader rmaReportReader) : IRmaReportExcelExporter
{
    ...
    private static string BuildWorksheetXml(IReadOnlyList<RmaReportRow> rows)
    {
        ...
        AppendRow(builder, 1, [.. RmaReportExportProjection.Headers]);

        for (var index = 0; index < rows.Count; index++)
        {
            AppendRow(builder, index + 2, RmaReportExportProjection.Project(rows[index]));
        }
    ...
    private const string WorkbookXml =
        ...
        <workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"
          ...
          <sheets>
            <sheet name="RMA Reports" sheetId="1" r:id="rId1"/>
          </sheets>
        </workbook>
        """;
