...
public sealed class RmaRegisterExcelExporter(
    IRmaRegisterReader rmaRegisterReader) : IRmaRegisterExcelExporter
{
    ...
    private static string BuildWorksheetXml(IReadOnlyList<RmaRegisterRow> rows)
    {
        var builder = new StringBuilder();
        builder.Append("<?xml version=\"1.0\" encoding=\"UTF-8\"?>");
        builder.Append("<worksheet xmlns=\"http://schemas.openxmlformats.org/spreadsheetml/2006/main\"><sheetData>");
        AppendRow(builder, 1, [.. RmaRegisterExportProjection.Headers]);

        for (var index = 0; index < rows.Count; index++)
        {
            AppendRow(builder, index + 2, RmaRegisterExportProjection.Project(rows[index]));
        }
    ...
    private const string WorkbookXml =
        ...
        <workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"
          ...
          <sheets>
            <sheet name="RMA Register" sheetId="1" r:id="rId1"/>
          </sheets>
        </workbook>
        """;
