...
public sealed class CustomerReportExcelExporter(
    ICustomerReportReader customerReportReader) : ICustomerReportExcelExporter
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        using (var archive = new ZipArchive(stream, ZipArchiveMode.Create, leaveOpen: true))
        {
            ...
            CreateEntry(
                ...
                UsesCustomerRows(filter)
                    ? BuildCustomerWorksheetXml(await customerReportReader.ListCustomersAsync(filter, cancellationToken))
                    : BuildRmaWorksheetXml(await customerReportReader.ListRmasAsync(filter, cancellationToken)));
        }
    ...
    private static string BuildCustomerWorksheetXml(IReadOnlyList<CustomerContractReportRow> rows)
    {
        var builder = new StringBuilder();
        builder.Append("<?xml version=\"1.0\" encoding=\"UTF-8\"?>");
        builder.Append("<worksheet xmlns=\"http://schemas.openxmlformats.org/spreadsheetml/2006/main\"><sheetData>");
        AppendRow(builder, 1, CustomerReportExportProjection.CustomerHeaders);

        for (var index = 0; index < rows.Count; index++)
        {
            AppendRow(builder, index + 2, CustomerReportExportProjection.Project(rows[index]));
        }
    ...
    private static string BuildRmaWorksheetXml(IReadOnlyList<CustomerSupportRmaReportRow> rows)
    {
        var builder = new StringBuilder();
        builder.Append("<?xml version=\"1.0\" encoding=\"UTF-8\"?>");
        builder.Append("<worksheet xmlns=\"http://schemas.openxmlformats.org/spreadsheetml/2006/main\"><sheetData>");
        AppendRow(builder, 1, CustomerReportExportProjection.RmaHeaders);
    ...
    private const string WorkbookXml =
        ...
        <workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"
          ...
          <sheets>
            <sheet name="Customer Reports" sheetId="1" r:id="rId1"/>
          </sheets>
        </workbook>
        """;
