...
public sealed class CustomerListExcelExporter(
    ICustomerListReader customerListReader) : ICustomerListExcelExporter
{
    ...
    private static string BuildWorksheetXml(IReadOnlyList<CustomerListItem> rows)
    {
        var builder = new StringBuilder();
        builder.Append("<?xml version=\"1.0\" encoding=\"UTF-8\"?>");
        builder.Append("<worksheet xmlns=\"http://schemas.openxmlformats.org/spreadsheetml/2006/main\"><sheetData>");
        AppendRow(builder, 1, [.. CustomerListExportProjection.Headers]);

        for (var index = 0; index < rows.Count; index++)
        {
            AppendRow(builder, index + 2, CustomerListExportProjection.Project(rows[index]));
        }
